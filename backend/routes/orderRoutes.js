...
router.post('/create', requireAuth, createOrderValidation, async (req, res) => {
    try {
        ...
        const order = new Order({
            ...
            paymentMethod: paymentMethod || 'bank_transfer', // changed
...
router.post('/:orderId/payment-proof', requireAuth, async (req, res) => {
    try {
        const { paymentProof } = req.body; // could be reference number or file path
        const order = await Order.findById(req.params.orderId);
        if (!order) {
        ...
        order.paymentStatus = 'paid'; // auto-approve for now
        await order.save();
